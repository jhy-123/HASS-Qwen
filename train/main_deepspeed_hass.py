...
def getkacc(model, data, head, rank, max_length=5):
    ...
    hidden_states = data["hidden_states"].to(rank)
...
for epoch in range(num_epochs):
    ...
    for batch_idx, data in enumerate(tqdm(train_loader)):
        ...
        hidden_states, input_ids, attention_mask, target, loss_mask = data["hidden_states"].to(rank), data["input_ids"].to(rank), data["attention_mask"].to(rank), data["target"].to(rank), data["loss_mask"][..., None].to(rank)
        loss = 0
        with torch.no_grad():
            target_head = head_engine(target)
            target_p = nn.Softmax(dim=2)(target_head)
            target_p = target_p.detach()

        q_hidden_states = None
        for forward_idx in range(args.forward_num_total):
            predict = model_engine(hidden_states, input_ids, attention_mask, q_hidden_states=q_hidden_states)
    ...
    if (epoch + 1) % train_config["save_freq"] == 0 or epoch == 0:
        ...
        for batch_idx, data in enumerate(tqdm(test_loader)):
            ...
            with torch.no_grad():
                if batch_idx < 10:
                    acces = getkacc(model, data, head_engine, rank, max_length=5)
                    for i in range(len(acces)):
                        k_acc[i].append(acces[i])

                q_hidden_states = None
                for forward_idx in range(args.forward_num_total):
                    predict = model_engine(data["hidden_states"].to(rank), input_ids=data["input_ids"].to(rank), attention_mask=data["attention_mask"].to(rank), q_hidden_states=q_hidden_states)
                    if q_hidden_states is None:
                        q_hidden_states = torch.cat([data["hidden_states"][:, :1, :].to(rank), predict[:, :-1, :]], dim=1)[None, :, :, :]
                    else:
                        new_q_hidden_states = torch.cat([q_hidden_states[-1][:, :1, :], predict[:, :-1, :]], dim=1)[None, :, :, :]
                        q_hidden_states = torch.cat([q_hidden_states, new_q_hidden_states], dim=0)
